...
if __name__ == '__main__':
    ...
    # file_name = ROOT / 'data' / 'Generations' / 'adult_generation_10_race.csv'
    ...
    # file_name = ROOT / 'data' / 'Generations' / 'bank-marketing_generation_10_age.csv'
    # df = pd.read_csv(file_name)
    # df['age'] = [1 if v == '25<=x<=65' else 0 for v in df['age']]
    # df['class-label'] = [1 if v == "yes" else 0 for v in df['class-label']]
    # df.to_csv(file_name, index=False)

    # file_name = ROOT / 'data' / 'Generations' / 'credit-card-clients_generation_10_SEX.csv'
    # df = pd.read_csv(file_name)
    # df['AGE'] = [1 if 25 <= v <= 65 else 0 for v in df['AGE']]
    # df['SEX'] = [1 if v == 1 else 0 for v in df['SEX']]
    # df.to_csv(file_name, index=False)
    #
    # file_name = ROOT / 'data' / 'Generations' / 'credit-scoring_generation_10_Sex.csv'
    # df = pd.read_csv(file_name)
    # df['Age'] = [1 if 25 <= v <= 65 else 0 for v in df['Age']]
    # df['Sex'] = [1 if v == 1 else 0 for v in df['Sex']]
    # df.to_csv(file_name, index=False)
    #
    # file_name = ROOT / 'data' / 'Generations' / 'german-credit-data_generation_10_sex.csv'
    # df = pd.read_csv(file_name)
    # df['age'] = [1 if 25 <= v <= 65 else 0 for v in df['age']]
    # df['sex'] = [1 if v == 'male' else 0 for v in df['sex']]
    # df.to_csv(file_name, index=False)

    # file_name = ROOT / 'data' / 'Origins' / 'dutch-census.csv'
    # df = pd.read_csv(file_name)
    # df['sex'] = [1 if v == 'male' else 0 for v in df['sex']]
    # df.to_csv(file_name, index=False)
