...
def none_normalization(data: pd.DataFrame, column: str):
    return data
...
def de_none_normalization(data: pd.DataFrame):
    return data
...
def norm(data: pd.DataFrame, columns: [], norm_types: []):
    ...
    for i in range(len(columns)):
        ...
        if norm_types[i] == 'none':
            data = none_normalization(data, columns[i])
...
def denorm(data: pd.DataFrame, columns: [], norm_types: [], norm_dict: dict):
    for i in range(len(columns)):
        ...
        if norm_types[i] == 'none':
            data = de_none_normalization(data)
